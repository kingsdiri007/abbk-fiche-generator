...
export function generatePDF(formData) {
  ...
  // Add logo image - try multiple paths
  try {
    // Try to load logo from public folder
    const logoPath = process.env.PUBLIC_URL + '/logo-abbk.png';
    doc.addImage(logoPath, 'PNG', logoX, logoY, logoWidth, logoHeight);
  } catch (error) {
    console.warn('Logo not found, using text fallback');
    doc.setFontSize(20);
    doc.setFont(undefined, 'bold');
    doc.text('ABBK PHYSICSWORKS', logoX, logoY + 50);
  }
  ...
  y += 12;
  doc.text('Branche: 4eme Etage Pépinière de Technopôle El', logoX, y);
  y += 12;
  doc.text('Ghazela, 2083 Rue Ibn Hamdoun, Tunisie.', logoX, y);
  y += 10;

  // Right side - Client information box
  ...
  // ===== FORMATION MODE: Multiple Formations - FIXED =====
  ...
  const formationData = formData.formationsData[formationId] || {};
      ...
      // Add spacing between formations
